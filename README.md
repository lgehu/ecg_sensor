...
| OUTPUT_FORMAT       | Set the output format of processed data. On ASCII mode,  the format is: <Time_stamp;Valued> where Time_stamp is the time difference since the board started in millisecond, value is the processed data of the selected stage. On binary format, every sample takes 8 bytes. First 4 bytes are the timestamp in ms. Next 4 bytes are the actual float value. Values are in big endian format. | OUT_ASCII &#124; FLOAT32 | OK or Invalid parameter |
| RESET               | Restart the board | None | OK or Invalid parameter |
| START               | Start automatic sampling on the selected input channel (default from flash) and send back result with the selected output format. During sampling, sensor parameters may not be applied and need to be restarted. | None | 
...
| VERSION             | Ask for the firmware version. | None | ECG_Sensor vX.X |
| SAMPLE_RATE         | Set the sample rate. | 0 < Integer_Value | OK or Invalid parameter |
...
| ENABLE_TRIGGER      | Enable or disable the trigger. For the ECG, the sensor will send data only if a peak is detected. Works either while auto sampling or using NEXT commands. | TRUE &#124; FALSE | OK or Invalid parameter |
