...
if __name__ == "__main__":
    ...
    with Serial(args.port, baudrate=115200, timeout=1) as ser:
        ...
        timestamp, value, is_pick = 0, 0, False
        for i in range(MAX_SAMPLE):

            if OUTPUT_FORMAT == "FLOAT32":
                timestamp = ecg_com.read_uint_32(ser)
                value = ecg_com.read_float_32(ser)
                is_pick = True if int(ser.read(1)[0]) == 1 else False
            else:
                rawdata = ecg_com.wait_response(ser)
                timestamp, value, pick = rawdata.split(";")
                is_pick = True if pick == 'TRUE' else False 

            values[i] = float(value)
            timestamps[i] = float(timestamp) * 0.001 # Convert ms in seconds
            
            if is_pick:
                pick_stamp.append(timestamps[i])
            
            print(timestamp, values[i], is_pick)
